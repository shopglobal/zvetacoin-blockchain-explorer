                        ...
                        bool tx_prove = false)
        {
            ...
            if (viewkey_str.empty())
            {
                if (!tx_prove)
                    ...
                    return string("Transaction private key not provided!");
            }
...
//        }
            ...
            if (!raw_tx_data.empty())
            {
                ...
                {
                    string msg = fmt::format("Cannot obtain tx_data_blob from raw_tx_data");
            ...
            {
                cerr << "Cannot get transaction in blockchain: " << tx_hash
                     << ". \n Check mempool now" << endl;
                ...
                {
                    // tx is nowhere to be found :-(
                    return string("Cannot get transaction: " + tx_hash_str);
                }
            }
            ...
            {
                cerr << "Cannot get block height: " << tx_hash
                     << e.what() << endl;
            }
            ...
            {
                cerr << "Cannot get block: " << tx_blk_height << endl;
            }
            ...
            {
                cerr << "Cannot get derived key for: "  << "\n"
                     << "pub_tx_key: " << pub_key << " and "
                     << "prv_view_key" << prv_view_key << endl;

                return string("Cannot get key_derivation");
            }
            ...
            for (pair<txout_to_key, uint64_t>& outp: txd.output_pub_keys)
            {
                ...
                if (mine_output && tx.version == 2)
                {
                    ...
                    if (!is_coinbase(tx))
                    {
                        ...
                        if (!r)
                        {
                            cerr << "\nshow_my_outputs: Cannot decode ringCT! " << endl;
                        }
            ...
            for (const txin_to_key& in_key: input_key_imgs)
            {
                ...
                for (const uint64_t& abs_offset: absolute_offsets)
                {
                    ...
                    {
                        cerr << "Cannot get transaction: " << tx_out_idx.first << endl;
...
//                        }
                        ...
                        if (mine_output && mixin_tx.version == 2)
                        {
                            ...
                            if (!is_coinbase(mixin_tx))
                            {
                                ...
                                if (!r)
                                {
                                    cerr << "show_my_outputs: key images: Cannot decode ringCT!" << endl;
                                }
...
//                                << endl;
        ...
        show_checkrawtx(string raw_tx_data, string action)
        {
            ...
            if (unsigned_tx_given)
            {
                ...
                {
                    cerr << "Failed to parse unsigned transaction data " << endl;
                }
                ...
                if (r)
                {
                    ...
                    for (const ::tools::wallet2::tx_construction_data& tx_cd: exported_txs.txes)
                    {
                        ...
                        for (size_t i = 0; i < no_of_sources; ++i)
                        {
                            ...
                            {
                                cerr << "Cannot get transaction from blockchain: " << real_toi.first << endl;
                                return string("Cannot get transaction: " + pod_to_hex(real_toi.first));
                            }
                            ...
                            for(const tx_source_entry::output_entry& oe: tx_source.outputs)
                            {
                                ...
                                {
                                    cerr << "Cannot get transaction from blockchain: " << toi.first
                                         << ". \n Check mempool now" << endl;
                                    // tx is nowhere to be found :-(
                                    return string("Cannot get transaction: " + pod_to_hex(toi.first));
                                }
                                ...
                                {
                                    cerr << "Cannot get block: " << txd.blk_height << endl;
                                    return string("Cannot get block: "  + to_string(txd.blk_height));
                                }
                ...
                {
                    cerr << "deserialization of unsigned transaction data NOT successful" << endl;
                    return string("deserialization of unsigned transaction data NOT successful. "
            ...
            else
            {
                ...
                if (strncmp(decoded_raw_tx_data.c_str(), SIGNED_TX_PREFIX, magiclen) != 0)
                {
                    ...
                    if (!epee::string_tools::parse_hexstr_to_binbuff(raw_tx_data, tx_data_blob))
                    {
                        string msg = fmt::format("The data is neither unsigned, signed transaction or raw transaction! "
                                                         "Its prefix is: {:s}",
                                                 data_prefix);
                    ...
                    context["data_prefix"] = string("none as this is pure raw transaction data");
                ...
                {
                    cerr << "Failed to parse signed transaction data " << endl;
                }

                if (!r)
                {
                    cerr << "deserialization of signed transaction data NOT successful" << endl;
                    return string("deserialization of signed transaction data NOT successful. "
                                          "Maybe its not base64 encoded?");
                }
                ...
                for (tools::wallet2::pending_tx& ptx: ptxs)
                {
                    ...
                    for (const tx_source_entry&  tx_source: ptx.construction_data.sources)
                    {
                        ...
                        if (!mcore->get_tx(real_toi.first, real_source_tx))
                        {
                            cerr << "Canont get transaction from blockchain: " << real_toi.first << endl;
                            return string("Cannot get transaction: " + pod_to_hex(real_toi.first));
                        }
        ...
        show_pushrawtx(string raw_tx_data, string action)
        {
            ...
            else
            {
                ...
                if (strncmp(decoded_raw_tx_data.c_str(), SIGNED_TX_PREFIX, magiclen) != 0)
                {
                    string error_msg = fmt::format("The data does not appear to be signed raw transaction! Data prefix: {:s}",
                                                   data_prefix);
                ...
                if (!r)
                {
                    string error_msg = fmt::format("Deserialization of signed transaction data NOT successful! "
                                                           "Maybe its not base64 encoded?");
            ...
            while (!ptx_vector.empty())
            {
                ...
                {
                    string error_msg = fmt::format("Transaction already exists in the mempool: {:s}\n",
                                                   tx_hash_str);
                ...
                {
                    string error_msg = fmt::format("Transaction already exists in the blockchain: {:s}\n",
                                                   tx_hash_str);
                ...
                {
                    string error_msg = fmt::format("Transaction with hash {:s} has already spent inputs\n",
                                                   tx_hash_str);
                ...
                {
                    string error_msg = fmt::format(
                            "Submitting signed transaction {:s} to daemon failed: {:s}\n",
        ...
        show_checkrawkeyimgs(string raw_data, string viewkey_str)
        {
            ...
            if (viewkey_str.empty())
            {
                string error_msg = fmt::format("View key not given. Cannot decode "
