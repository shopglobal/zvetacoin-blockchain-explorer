...
MempoolStatus::read_mempool()
{
    ...
    for (size_t i = 0; i < mempool_tx_info.size(); ++i)
    {
        ...
        if (epee::string_tools::hex_to_pod(_tx_info.id_hash, mem_tx_hash))
        {
            ...
            last_tx.fee_str         = xmreg::xmr_amount_to_str(_tx_info.fee, "{:0.6f}", false);
            last_tx.xmr_inputs_str  = xmreg::xmr_amount_to_str(last_tx.sum_inputs , "{:0.6f}");
            last_tx.xmr_outputs_str = xmreg::xmr_amount_to_str(last_tx.sum_outputs, "{:0.6f}");
...
bf::path MempoolStatus::blockchain_path {"~/.zvetacoin/lmdb"};
string MempoolStatus::deamon_url {"http:://127.0.0.1:18191"};
